...
public class LogisticDao {
	...
	public int getCount(String field,String keyword) { //총 목록개수
		...
		try {
			con=DBCPBean.getConn();
			String sql ="SELECT NVL(COUNT(LOGIID),0) FROM LOGISTIC NATURAL JOIN MEMBERINFO NATURAL JOIN ITEMINFO \r\n" + 
					"WHERE LOGIID IN(SELECT NVL(MAX(LOGIID),0) MAXLOGI FROM LOGISTIC GROUP BY MEMID,LOGIINFO)";
			if(field!=null && !field.equals("")) {
